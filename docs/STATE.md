...
### Character system unification + Higgsfield sheets — DESIGNED 2026-08-12 (spec approved, not built)

Full-unification refactor (Approach C, chosen over targeted cleanup): the nine competing representations of "a character" (registry record · hand-copied client mirror · LoRA sidecar · `sailor_character*` node props · `CastMember` · materialized `Ref` · `LipSyncSheet.face` · `TrainerSeed` · free-text `subject`) collapse into one shared `CharacterState` model in `frontend/shared/`. Variants become **states** (Cal-clean / Cal-wet / Cal-bloody, per Higgsfield's one-asset-per-state rule); each state generates five panels — **headless** full-body front + back, large ¾ portrait, smile + no-smile face close-ups — client-baked into one composite sheet that is THE identity asset **both image and video generation consume identically** (cap-of-1 ref rule survives: one image, but it carries front/back/face). The silent LoRA-vs-ConsistentFace image fork is removed (LoRA → visible opt-in); the state descriptor finally reaches the shot prompt clause. A state must survive a **manual 10-tile stress test (10/10 recognizable)** to reach `locked`; edits revert to `draft`. Also lands: single store replacing five window CustomEvents, per-state PATCH + `updatedAt` 409 guard replacing full-array replace + five stale-closure guards, auto-spend-on-mount removed. Methodology: Higgsfield's Seedance character-sheet guide. Spec: [2026-08-12-character-system-unification-design.md](superpowers/specs/2026-08-12-character-system-unification-design.md) (`f5a09ee15`).
