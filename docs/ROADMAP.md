...
**Status (2026-08-24): the legacy-retrofit proof is DONE.** Option 3 happened, staged organically rather than by decision: Gradient's seven declaration sites collapsed to one — agent, motion, and sweeps had been deriving from `GRADIENT_CONTROLS` since the control-schema pass, and the last consumer, the inspector itself, now renders from it too (−723 lines of hand-written panel; presentation remap keeps on-screen titles/order without touching schema order). 3D Studio followed for Material/Camera/Lighting/Background (Transform waits on a soft-range row — its shipped inputs are unbounded). The "done when" clause below is satisfied on both halves: new studios ship all four capabilities from one declaration (Vector Type proved it; Shape/post re-proved it), and a legacy studio has been retrofitted against real complexity. The same-day follow-up (2026-08-24, `2026-08-24-scene3d-full-derivation-design.md`) closed the last three gaps: Scene3D's panel is permissive (novel schema keys draw + write + read), Transform migrated on new soft-range rows (bounds describe, entry unclamped), selects carry display labels, and Geometry/Light/Decal joined the schema quietly (zero vocabulary growth). What Act 1 still owes: the deliberate GRANTS — deciding which of the quiet Scene3D entries the agent and motion get. See `docs/superpowers/specs/2026-08-24-derived-inspector-retrofit-design.md`.

**Open decision (paused 2026-07-25, now resolved by events — kept for the record):** how to scope the first proof.
...
**Scene3D — clay sculpting and shape merging — LANDED 2026-08-04.** Not one of the three chosen absorption families; a depth expansion of the existing Scene3D surface rather than a new one, so it doesn't move the "done when" bar above. ~30 commits (`e6b5392d0`..`26fbec850`). A new `mesh` `PrimitiveKind` (geometry from stored vertices, inheriting every modifier/material/motion/bake path for free) underpins all of it: a delta-encoded vertex codec small enough to live inline in the scene document (~5x smaller than naive quantise-and-deflate), an async decode cache, a "To mesh" freeze action, and a cloner budget clamp. On top of that substrate, all four phases are now complete — a voxel module (mesh → SDF → surface nets, signed by exterior flood fill) powering Remesh/Solidify, seven sculpt brushes with symmetry and per-stroke undo, and boolean Merge with a blend fillet. **Sculpting itself now works: drag on a mesh and it deforms under the cursor.** Known rough edges: Remesh takes ~2s at default resolution and ~10s at the 128 maximum (a Web Worker is the next lever), and merging softens sharp edges at grid resolution by design. See `docs/STATE.md` for the full write-up.
