...
- Parity harness (Phase 0): `PreviewRenderer` interface, `/timeline-harness` page, fixtures + committed goldens, Playwright golden spec, Python ground truth via `render_frame_np` and `/sailor/timeline/render_frame`.
...
3. **M3 — Editor flag.** 🚧 Implementation landed 2026-06-09 (plan: docs/plans/2026-06-09-phase1-m3-editor-flag-plan.md); dogfooding IN PROGRESS per docs/plans/2026-06-09-phase1-m3-dogfooding-checklist.md. usePlaybackEngineGL behind `sailor:Engine.WebGLPreview` (default off), Canvas2D auto-fallback, kinetic/title/sequence/audio wired, per-clip skip-and-warn, WebCodecs size cap, video loop parity with the exporter, harness pages dev-gated.
