...
- **Legacy `maskUrl` (per-slot mask):** decide during migration — carry it into `layer.maskStrokes`/`maskedByKey` equivalents or keep reading the slot registry for schema-2 frames; do not silently drop it.
