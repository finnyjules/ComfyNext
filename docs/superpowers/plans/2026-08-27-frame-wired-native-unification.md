...
- **Sentinel finalizer preserves scale:** a migrated layer with `w: -1` (dims unknown at migration) still has its `layer{N}_scale` widget intact; the first-paint finalizer must set `w = fit * scale` (via `wiredBoxFromWidgets` with the surviving widget transform), not `fit` alone. Pin with a test. Undo hazard: undoing back into a sentinel state is a sync no-op, so widgets keep post-finalize values and the finalizer would re-derive the transform the user just undid — make the finalizer skip layers whose widgets already match, or record finalization as part of the same history step.
- **lastAspect refresh (Task 4 review carry-over):** paint is read-only and never writes `lastAspect` back, and the write-through's dims fallback uses `lastAspect` — so after an upstream re-run at a new aspect, preview re-fits live while the widget `scale` drifts PERMANENTLY unless the host reconciles. The host must watch slot content dims and (a) update `layer.lastAspect` and (b) pass real `wiredDims` into the editor/sync when content changes. Pin with a test.
...
- [ ] **Step 1: Failing unit test** — mixed selection: align-left puts both layers' left edges equal; nudge moves both; duplicate of the mix produces a snapshot image layer for the wired member (assert the copy's `kind` is `image`, not `wired`). NOTE (Task 4 review): today `duplicateLayers` (`lib/compositor/layerEdits.ts:39-43`) deep-copies and preserves `slot`, and `layerClipboard.ts` has no wired handling — BOTH paths must convert wired→snapshot (or refuse), else two layers share one slot and the widget write-through silently last-writes-wins while the server renders one copy. `syncAllWiredWidgets` should also warn on duplicate slots as a backstop.
...
- [ ] **Step 3b (Task 5 review carry-overs):** wired resize becomes corner-ANCHORED aspect-locked (Task 5 shipped width-only uniform-from-center; the Figma feel needs the grabbed corner to move and the opposite corner to stay — reuse the anchored math from the rect/image resize path with the aspect lock on top). Card shows the `unlinked` badge (modal-only today). Card's editor gets `materializeWired` passed so ⌘D parity holds if the card ever wires `handleEditorKey`.
