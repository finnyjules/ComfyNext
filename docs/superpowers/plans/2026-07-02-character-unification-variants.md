...
- Event naming `comfynext:<name>`; mutating registry paths dispatch `comfynext:charactersChanged`.
...
- Produces:
  ...
  - `CharacterNode.vue`: variant `<select>` shown when the picked character has >1 variant; writes `properties.comfynext_characterVariantId`; dispatches `comfynext:castEdgesChanged` on change.
