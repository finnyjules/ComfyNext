...
- Produces (later tasks rely on these exact names):
  ...
  - `effectiveBrand(...)`: merges `palette` (later non-empty array replaces whole array) and `roles` (per-role, later wins); materializes each role key from `roles`→palette entry hex (fallback: merged legacy flat value); adds flat token keys `palette.<slug>` = hex onto the result for **explicit** palette entries so `{{ brand.palette.<slug> }}` resolves via the existing flat-first lookup. *(Amended during execution: virtual/legacy palettes do NOT get flat token keys — doing so breaks the pinned back-compat contract `effectiveBrand(t) toEqual t` for legacy kits, and no surface mints palette tokens for legacy kits: the bind buttons and agent hints read `effectiveBrand.palette`, absent until a kit is migrated.)*
