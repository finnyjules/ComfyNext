...
**Architecture:** Three seams, all existing: (1) chip data joins `app/data/action-catalog.ts` as `CHIPS_BY_DOMAIN`; (2) chip clicks reuse the `comfynext:applyEffect` window event (VueNodeCanvas's `handleApplyEffect` at ~1676 is already output-generic — `output || 'IMAGE'` — and honors `branch: true` so a chip branches off the artifact rather than re-pointing its chain); (3) a new `comfynext:openActions` event lets the canvas open the Actions panel with a target domain (new prop seam into `GeneratorsPanel.vue`, which today keeps `activeDomain` internal).
...
- All commands from `/Users/julien/Documents/GitHub/ComfyNext/frontend`.
...
### Task 2: `comfynext:openActions` seam — canvas → Actions panel with domain
...
- Produces: window event contract `comfynext:openActions` with `detail: { domain?: 'image'|'audio'|'video'|'3d'|'text' }`; `GeneratorsPanel` prop `focusDomain?: { domain: ActionDomain; ts: number } | null`.
...
Register/unregister with the layout's existing window-listener lifecycle (find where other `comfynext:*` listeners or mount hooks live in this file — mirror that): `window.addEventListener('comfynext:openActions', handleOpenActions)` in `onMounted`, matching remove in `onUnmounted`.
...
- [ ] **Step 3: Manual sanity + commit** — with a dev server running, in the browser console: `window.dispatchEvent(new CustomEvent('comfynext:openActions', { detail: { domain: 'video' } }))` → panel opens on Video tab. If no server is running note it for the controller's verify task.

```bash
git add app/layouts/default.vue app/components/vue-canvas/GeneratorsPanel.vue
git commit -m "feat(ia): comfynext:openActions seam — open Actions panel on a target domain"
```
...
**Interfaces:**
- Consumes: `CHIPS_BY_DOMAIN` (Task 1), `comfynext:applyEffect` (existing, `branch: true` + `focus: true`), `comfynext:openActions` (Task 2), `getGeneratorIcon` from `~/data/generator-icons`, `ACTION_CATALOG` for tooltips.
