...
## What the stage built (7 task commits + 3 live-verification fixes; base 6a0d39f85)
...
- **A refusal through a real paid submission** (the toast was driven through the real postMessage boundary but not from a real `/prompt` 400 — needs `OPENAI_API_KEY` or the admin pause; pairs with the owed Stage-7 checklist).
