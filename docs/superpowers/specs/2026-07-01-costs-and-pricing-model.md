...
- **Launch: serverless per-second GPU — RunPod Serverless (L40S $1.91/hr, first-party `worker-comfyui` image, network-volume model cache), Modal as the engineering-quality alternative (A100 $2.50/hr, <3 s container starts via memory snapshots).** An always-on L40S is ~$713/mo and only breaks even at ~25% utilization — far above launch traffic.
