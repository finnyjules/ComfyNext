...
4. `NextStepsStrip` renders fix chips (amber accent — matches the existing `fromReview` amber tag idiom, not purple) ahead of the standard suggestion chips. Fix chips are **sticky**: exempt from the 12s auto-dismiss (reviews arrive ~10s after render by nature); cleared on click, explicit dismiss, or a newer take on that node.
...
- **`components/vue-canvas/NextStepsStrip.vue`** — renders fix chips (amber text/border, `⚠`-style lucide icon) before the standard chips; fix chips ignore the auto-dismiss timer (timer only clears the generic chips).
