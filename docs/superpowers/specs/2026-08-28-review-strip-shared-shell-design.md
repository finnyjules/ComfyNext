...
- **`sketch-review-strip.unit.spec.ts` is UPDATED for the intended interaction change** — the sketch strip's *presentation* deliberately changes (Keep moves from the actions bar onto the tile, revealed on hover; the `sketch-tip` hover-preview popup is removed), so the tests tied to those two things are rewritten/removed. But its **emitted-event contract is preserved** (`hover/select/keep/cancel/reroll/dropAt` still fire with the same payloads, same `data-testid`s for the tiles/ghost/actions), which is what keeps `VueNodeCanvas.vue` untouched — assert that the events are unchanged even as the trigger UI moves.
