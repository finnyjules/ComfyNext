#!/bin/bash
# Extra ComfyUI workers for the ComfyNext parallel-run pool.
#
...
#
# Logs: /tmp/comfynext-worker-<port>.log
set -u
cd "$(dirname "$0")/.."
...
for PORT in 8189 8190 8191 8192; do
  if curl -s -o /dev/null --max-time 1 "http://127.0.0.1:$PORT/"; then
    ...
    nohup .venv/bin/python main.py --listen 127.0.0.1 --port "$PORT" \
      > "/tmp/comfynext-worker-$PORT.log" 2>&1 &
