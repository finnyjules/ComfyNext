...
describe('SHAPE_CONTROLS integrity', () => {
  ...
  it('every slider default equals the value DEFAULT_CONFIG actually ships', () => {
    ...
    //
    // NO exceptions. Task 8 briefly holed this test for post.grainAmount /
    // post.grainSize; the hole was hiding a real defect (mergeConfig re-deriving
    // grain from the legacy style.grain on EVERY load, so no control could change
    // it). If a key needs excluding here again, treat that as the bug report it is.
    const c = cfg()
    const params = makeConfigParams(() => c, () => 0)
