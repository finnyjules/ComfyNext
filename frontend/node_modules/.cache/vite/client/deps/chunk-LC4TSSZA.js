...
function onClickOutside(target, handler, options = {}) {
  ...
  const shouldIgnore = (event) => {
    return toValue2(ignore).some((target$1) => {
...
function useMediaQuery(query, options = {}) {
  ...
  watchEffect2(() => {
    ...
    if (!isSupported.value) return;
    mediaQuery.value = window$1.matchMedia(toValue2(query));
...
function useBrowserLocation(options = {}) {
  ...
  for (const [key, ref$1] of objectEntries(refs)) watch2(ref$1, (value) => {
...
function useStorage(key, defaults$1, storage, options = {}) {
  ...
  const data = (shallow ? shallowRef2 : ref2)(typeof defaults$1 === "function" ? defaults$1() : defaults$1);
  ...
  const rawInit = toValue2(defaults$1);
...
function useCssSupports(...args) {
  ...
  return { isSupported: computed2(() => {
    ...
    return args.length === 2 ? window$1 === null || window$1 === void 0 ? void 0 : window$1.CSS.supports(toValue2(prop), toValue2(value)) : window$1 === null || window$1 === void 0 ? void 0 : window$1.CSS.supports(toValue2(prop));
  }) };
}
...
function useDocumentVisibility(options = {}) {
  const { document: document$1 = defaultDocument } = options;
  if (!document$1) return shallowRef2("visible");
  const visibility = shallowRef2(document$1.visibilityState);
...
function useDraggable(target, options = {}) {
  ...
  const handleEvent = (e) => {
    if (toValue2(preventDefault$1)) e.preventDefault();
  ...
  if (isClient) {
    const config = () => {
      ...
      return {
        ...
        passive: !toValue2(preventDefault$1)
      };
    };
...
function useElementByPoint(options) {
  ...
  const isSupported = useSupported(() => {
    if (toValue2(multiple)) return document$1 && "elementsFromPoint" in document$1;
  ...
  return {
    ...
    ...scheduler(() => {
      ...
      element.value = toValue2(multiple) ? (_document$elementsFro = document$1 === null || document$1 === void 0 ? void 0 : document$1.elementsFromPoint(toValue2(x), toValue2(y))) !== null && _document$elementsFro !== void 0 ? _document$elementsFro : [] : (_document$elementFrom = document$1 === null || document$1 === void 0 ? void 0 : document$1.elementFromPoint(toValue2(x), toValue2(y))) !== null && _document$elementFrom !== void 0 ? _document$elementFrom : null;
    })
  };
}
...
function useElementVisibility(element, options = {}) {
  ...
  const elementIsVisible = shallowRef2(initialValue);
...
function useFetch(url, ...args) {
  ...
  watch2([refetch, toRef(url)], ([refetch$1]) => refetch$1 && execute(), { deep: true });
  const shell = {
  ...
  function setMethod(method) {
    return (payload, payloadType) => {
      if (!isFetching.value) {
        ...
        if (isRef2(config.payload)) watch2([refetch, toRef(config.payload)], ([refetch$1]) => refetch$1 && execute(), { deep: true });
        return {
          ...shell,
          then(onFulfilled, onRejected) {
...
function useFileDialog(options = {}) {
  ...
  const applyOptions = (options$1) => {
    ...
    if (!el) return;
    el.multiple = toValue2(options$1.multiple);
    el.accept = toValue2(options$1.accept);
    el.webkitdirectory = toValue2(options$1.directory);
    if (hasOwn(options$1, "capture")) el.capture = toValue2(options$1.capture);
  };
...
function useMagicKeys(options = {}) {
  ...
  const proxy = new Proxy(refs, { get(target$1, prop, rec) {
    ...
    if (!(prop in refs)) if (/[+_-]/.test(prop)) {
      ...
      refs[prop] = computed2(() => keys$1.map((key) => toValue2(proxy[key])).every(Boolean));
...
function useMouseInElement(target, options = {}) {
  ...
  const targetRef = shallowRef2(target !== null && target !== void 0 ? target : window$1 === null || window$1 === void 0 ? void 0 : window$1.document.body);
...
function useMousePressed(options = {}) {
  ...
  const target = computed2(() => unrefElement(options.target) || window$1);
  const listenerOptions = {
    passive: true,
    capture
  };
...
function useNavigatorLanguage(options = {}) {
  ...
  const language = shallowRef2(navigator$1 === null || navigator$1 === void 0 ? void 0 : navigator$1.language);
...
function useClamp(value, min, max) {
  ...
  return computed2({
    ...
    set(value$1) {
      _value.value = clamp(value$1, toValue2(min), toValue2(max));
    }
  });
}
...
function useParallax(target, options = {}) {
  ...
  const orientation = reactive2(useDeviceOrientation({ window: window$1 }));
  const screenOrientation = reactive2(useScreenOrientation({ window: window$1 }));
...
function usePreferredLanguages(options = {}) {
  const { window: window$1 = defaultWindow } = options;
  if (!window$1) return shallowRef2(["en"]);
  const navigator$1 = window$1.navigator;
  const value = shallowRef2(navigator$1.languages);
  useEventListener(window$1, "languagechange", () => {
...
function useScriptTag(src, onLoaded = noop, options = {}) {
  ...
  const loadScript = (waitForScriptLoad) => new Promise((resolve, reject) => {
    ...
    let el = document$1.querySelector(`script[src="${toValue2(src)}"]`);
    if (!el) {
  ...
  const unload = () => {
    ...
    const el = document$1.querySelector(`script[src="${toValue2(src)}"]`);
...
function useSpeechSynthesis(text, options = {}) {
  ...
  const bindEventsForUtterance = (utterance$1) => {
    utterance$1.lang = toValue2(lang);
    utterance$1.voice = toValue2(options.voice) || null;
    utterance$1.pitch = toValue2(pitch);
    utterance$1.rate = toValue2(rate);
    utterance$1.volume = toValue2(volume);
    utterance$1.onstart = () => {
...
function useTextSelection(options = {}) {
  ...
  const { window: window$1 = defaultWindow } = options;
  const selection = shallowRef2((_window$getSelection = window$1 === null || window$1 === void 0 ? void 0 : window$1.getSelection()) !== null && _window$getSelection !== void 0 ? _window$getSelection : null);
...
function useUrlSearchParams(mode = "history", options = {}) {
  ...
  if (!window$1) return reactive2(initialValue);
...
function useWindowFocus(options = {}) {
  const { window: window$1 = defaultWindow } = options;
  if (!window$1) return shallowRef2(false);
  const focused = shallowRef2(window$1.document.hasFocus());
  const listenerOptions = { passive: true };
...
//# sourceMappingURL=chunk-LC4TSSZA.js.map
