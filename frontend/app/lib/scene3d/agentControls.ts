...
MATERIAL: \`object.material.type\` picks the shading model (standard/phong/toon/matcap/glass/fresnel/gradient/image/shaderFill); most of the other material sliders only apply to SOME types and are withheld from the control list otherwise (roughness/metalness need a PBR type, clearcoat/sheen/transmission need standard or glass, shininess/specular are phong-only) — never set a control that isn't offered for the object's current type. \`object.material.color\` is the base colour on standard/glass/phong/toon/fresnel; the other types derive colour a different way (matcap id, gradient ramp, image, shader) and ignore it.
...
LIGHTING/CAMERA/POST are scene-wide: \`lighting.preset\` picks the overall setup, \`sunAzimuth/sunElevation/sunIntensity/ambient\` tune it further; \`camera.fov\` is the lens; \`post.*\` are numeric-only render effects (bloom/grain/halftone/dot-screen/exposure/contrast/saturation/hue/chroma/blur/ambient-occlusion strength) — their on/off toggles are not agent-controllable, only their strength once already enabled in the studio.`
