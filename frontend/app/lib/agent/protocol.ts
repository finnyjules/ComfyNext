...
export function buildResultReviewPrompt(snapshot: SurfaceSnapshot, intent: string): string {
  ...
  return [
    ...
    `Commands you may propose as fixes:\n${commands}`,
    'Look at the ATTACHED IMAGE and judge ONLY whether it ACHIEVES THE REQUEST. Flag only concrete problems you can SEE: the content/subject is wrong or missing; a requested style or effect is weak or absent; the subject is cropped or cut off at an edge; visible artifacts/distortion; wrong colours; obvious low quality or blur; the wrong number of things; and ESPECIALLY botched ANATOMY — hands with the wrong number of fingers, extra/missing/fused limbs, mangled faces or eyes (look closely at hands, they fail most often). Do NOT invent problems, and do NOT impose any style, polish, or "design" the user did not ask for. Then propose the FEWEST fixes using the commands. For a STYLE/strength miss: setWidget to raise a scale/strength, swap a model, or edit the prompt. For ANATOMY or content defects, the best fixes are: (a) RE-ROLL — setWidget the generating node\'s "seed" to a different number (a fresh roll often just doesn\'t have the defect); or (b) add an EditImageNode wired from the result with a precise corrective instruction (e.g. "fix the left hand to have five normal fingers", "repair the distorted face") — and for faces specifically a FixFaces node also helps. If the image already achieves the request, return empty issues and empty fixes.',
