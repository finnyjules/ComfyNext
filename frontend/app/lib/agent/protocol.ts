...
export function buildResultReviewPrompt(snapshot: SurfaceSnapshot, intent: string): string {
  ...
  return [
    ...
    `Commands you may propose as fixes:\n${commands}`,
    'Look at the ATTACHED IMAGE and judge ONLY whether it ACHIEVES THE REQUEST. Flag only concrete problems you can SEE: the content/subject is wrong or missing; a requested style or effect is weak or absent; the subject is cropped or cut off at an edge; visible artifacts/distortion; wrong colours; obvious low quality or blur; the wrong number of things; and botched ANATOMY ANYWHERE in the frame. Scan EVERY figure — the main subject AND any people in the background — and check the WHOLE body of each, not just the hands: hands/fingers, feet/toes, arms, legs, necks, faces/eyes, and overall proportions. Flag the wrong count of digits or limbs, extra/missing/duplicated parts, fused or merged parts, contorted or impossible joints/poses, and mangled or distorted features. Hands fail most often so check them first, but do NOT stop there — feet/toes, limbs, and background figures are just as commonly broken; look deliberately at each before concluding. Do NOT invent problems, and do NOT impose any style, polish, or "design" the user did not ask for. Then propose fixes. For a STYLE/strength miss: setWidget to raise a scale/strength, swap a model, or edit the prompt. For a LOCAL defect — a botched hand/foot/face/limb (on ANY figure), an unwanted object, or a small wrong detail in an OTHERWISE-GOOD image — strongly PREFER a SURGICAL repair that keeps the rest intact: addNode an `EditImageNode` (Nano Banana / Flux Kontext — natural-language image editing) and connect the result image into its `input_image`, then setWidget its prompt to a precise corrective instruction naming WHICH defect on WHICH figure (e.g. "fix the right hand to have five clearly separated fingers", "fix the foot in the background to have five natural toes"; "keep everything else exactly the same"). If there are SEVERAL local defects, list them all in that ONE EditImageNode prompt rather than adding multiple nodes. The EditImageNode\'s OUTPUT is a NEW result — connect ONLY the source image INTO its input_image; NEVER wire its output back into the source generator or any upstream node (that makes an invalid loop). Leave its output unconnected; a result card is added automatically. For face-only defects a `FixFacesNode` (connect the result into its `image` input) also works. Do NOT re-roll a merely-local flaw — re-rolling discards the good image and changes the whole composition, which is almost never what the user wants. Only RE-ROLL (setWidget the generating node\'s "seed" to a different number) when the OVERALL result misses the request — wrong subject, scene, or style — not for a fixable local flaw. CRITICAL — never mix repair strategies on one image: a localized edit and a whole-image regeneration (editing the prompt, re-rolling the seed, or swapping the model) CONTRADICT each other. Pick ONE path per image: either the result is fundamentally wrong (regenerate) or it only has fixable local defects (edit them with one EditImageNode, no regeneration). And do NOT propose a prompt edit for minor wording or content nuances the user did not ask about (for example holding-vs-eating pizza) — only when a requested element is genuinely wrong or missing. If the image already achieves the request, return empty issues and empty fixes.',
