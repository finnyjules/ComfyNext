...
export function buildResultReviewPrompt(snapshot: SurfaceSnapshot, intent: string): string {
  ...
  return [
    ...
    `Commands you may propose as fixes:\n${commands}`,
    'Look at the ATTACHED IMAGE and judge ONLY whether it ACHIEVES THE REQUEST. Flag only concrete problems you can SEE: the content/subject is wrong or missing; a requested style or effect is weak or absent; the subject is cropped or cut off at an edge; visible artifacts/distortion; wrong colours; obvious low quality or blur; the wrong number of things; and ESPECIALLY botched ANATOMY — hands with the wrong number of fingers, extra/missing/fused limbs, mangled faces or eyes (look closely at hands, they fail most often). Do NOT invent problems, and do NOT impose any style, polish, or "design" the user did not ask for. Then propose fixes. For a STYLE/strength miss: setWidget to raise a scale/strength, swap a model, or edit the prompt. For a LOCAL defect — a botched hand/face/limb or a small wrong detail in an OTHERWISE-GOOD image — strongly PREFER a SURGICAL repair that keeps the rest intact: add an EditImageNode wired from the result with a precise corrective instruction (e.g. "fix the left hand to have five normal, well-defined fingers, keep everything else exactly the same"); for faces specifically a FixFaces node also works. Do NOT re-roll a merely-local flaw — re-rolling discards the good image and changes the whole composition, which is almost never what the user wants. Only RE-ROLL (setWidget the generating node\'s "seed" to a different number) when the OVERALL result misses the request — wrong subject, scene, or style — not for a fixable local flaw. If the image already achieves the request, return empty issues and empty fixes.',
