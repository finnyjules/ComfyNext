...
export function buildResultReviewPrompt(snapshot: SurfaceSnapshot, intent: string): string {
  ...
  return [
    ...
    `Commands you may propose as fixes:\n${commands}`,
    'Look at the ATTACHED IMAGE and judge ONLY whether it ACHIEVES THE REQUEST. Flag only concrete problems you can SEE: the content/subject is wrong or missing; a requested style or effect is weak or absent; the subject is cropped or cut off at an edge; visible artifacts/distortion; wrong colours; obvious low quality or blur; the wrong number of things; and ESPECIALLY botched ANATOMY — hands with the wrong number of fingers, extra/missing/fused limbs, mangled faces or eyes (look closely at hands, they fail most often). Do NOT invent problems, and do NOT impose any style, polish, or "design" the user did not ask for. Then propose fixes. For a STYLE/strength miss: setWidget to raise a scale/strength, swap a model, or edit the prompt. For BOTCHED ANATOMY (mangled hands/faces/limbs — the most common defect), the best fix is `fixAnatomy`: give the defect\'s kind ("hand"/"face"/"limb"), a tight bbox [x,y,w,h] in 0..1 fractions locating it, and a short note of what is wrong. It repairs ONLY that region and leaves the rest of the image untouched. Use it for EVERY hand/face/limb defect you can point to. Only if you cannot localize the defect, fall back to a RE-ROLL — setWidget the generating node\'s "seed" to a different number. Do NOT re-roll a merely-local flaw — re-rolling discards the good image and changes the whole composition, which is almost never what the user wants. Only RE-ROLL when the OVERALL result misses the request — wrong subject, scene, or style — not for a fixable local flaw. CRITICAL — never mix repair strategies on one image: a localized `fixAnatomy` repair and a whole-image regeneration (editing the prompt, re-rolling the seed, or swapping the model) CONTRADICT each other, because regenerating throws away the exact region you just repaired. So whenever you propose `fixAnatomy`, it MUST be the ONLY fix you return — do NOT also edit the prompt, change the seed, or swap the model for that same image. Pick ONE path per image: either the result is fundamentally wrong (regenerate, and then do not propose fixAnatomy at all) or it only has fixable local defects (repair them with fixAnatomy alone, no regeneration). And do NOT propose a prompt edit for minor wording or content nuances the user did not ask about (for example holding-vs-eating pizza) — only when a requested element is genuinely wrong or missing. If the image already achieves the request, return empty issues and empty fixes.',
