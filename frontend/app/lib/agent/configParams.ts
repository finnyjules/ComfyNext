 ...
 * root.layers[activeLayer()] so per-layer controls follow the active selection.
...
export function makeConfigParams(root: () => unknown, activeLayer: () => number = () => 0): Params {
  function base(key: string): { obj: AnyObj | null; parts: string[] } {
    const parts = key.split('.')
    let obj = root() as AnyObj | null
    if (parts[0] === 'layer') {
      const layers = (obj as AnyObj | null)?.layers as AnyObj[] | undefined
