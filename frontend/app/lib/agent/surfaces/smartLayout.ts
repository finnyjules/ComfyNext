...
const SMART_LAYOUT_COMMANDS: CommandSpec[] = [
  ...
  { op: 'setElementStyle', hint: 'Change the VISUAL STYLE of any element via its style object. target = element id; args: { patch }. Valid keys by type — text: fontFamily, fontWeight (400|700), fontSize (px), color, align (left|center|right), valign (top|middle|bottom), lineHeight, letterSpacing, transform (none|uppercase), panel; image: fit (cover|contain|stretch), borderRadius; shape: fill, borderColor, borderWidth, borderRadius. shape.fill accepts a solid colour OR a CSS gradient e.g. "linear-gradient(135deg,#FF6EB4,#FF8C42)". Any colour or font value may instead be a BRAND TOKEN bound to the kit — colours: "{{ brand.primary|secondary|accent|accent2|foreground|background }}", fonts: "{{ brand.fontDisplay|fontBody }}". Named palette colours bind as "{{ brand.palette.<name> }}" (see palette.* entries in the brand context). (For a plain text colour, setTextColor is simpler.)' },
