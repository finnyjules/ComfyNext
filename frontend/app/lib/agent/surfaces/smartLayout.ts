...
const STYLE_KEYS: Record<ElementV2['type'], Set<string>> = {
  text: new Set(['fontFamily', 'fontWeight', 'fontSize', 'color', 'align', 'valign', 'lineHeight', 'letterSpacing', 'transform', 'panel']),
...
const PROP_KEYS: Record<ElementV2['type'], Set<string>> = {
  text: new Set(['region', 'priority', 'bleed', 'hidden', 'locked', 'level', 'overflow', 'maxLines']),
...
const SMART_LAYOUT_COMMANDS: CommandSpec[] = [
  ...
  { op: 'setTextColor', hint: 'Set the COLOUR of a specific text element. target = element id; args: { color } — a hex "#RRGGBB" OR a brand token like "{{ brand.foreground }}" / "{{ brand.primary }}" to bind it to the kit. Use for "make the text yellow" / "the headline red" / "use the brand colour" — NOT setBrand (which retints the whole kit).' },
  { op: 'setElementStyle', hint: 'Change the VISUAL STYLE of any element via its style object. target = element id; args: { patch }. Valid keys by type — text: fontFamily, fontWeight (400|700), fontSize (px), color, align (left|center|right), valign (top|middle|bottom), lineHeight, letterSpacing, transform (none|uppercase), panel; image: fit (cover|contain|stretch), borderRadius; shape: fill, borderColor, borderWidth, borderRadius. shape.fill accepts a solid colour OR a CSS gradient e.g. "linear-gradient(135deg,#FF6EB4,#FF8C42)". Any colour or font value may instead be a BRAND TOKEN bound to the kit — colours: "{{ brand.primary|secondary|accent|accent2|foreground|background }}", fonts: "{{ brand.fontDisplay|fontBody }}". (For a plain text colour, setTextColor is simpler.)' },
  { op: 'setElementProps', hint: 'Change non-style PROPERTIES of any element. target = element id; args: { patch }. Valid keys — all: region ({col,colSpan,row,rowSpan}, 1-based on the master fine grid — see document.grid {cols,rows}; full-canvas = col 1..cols, row 1..rows — moves/resizes), priority (1=most important), bleed (bool), hidden (bool — show/hide), locked (bool); text also: level (caption|body|subhead|headline|display), overflow (shrink|shrink-then-truncate|grow), maxLines; image also: content (URL or {{ props.* }}), focal, collapse; shape also: shape (rect|circle). To change text COPY use setText.' },
