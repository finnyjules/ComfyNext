...
import { normalizeStateId, draftBadge } from '#shared/characters/types'
...
const {
  characters, loading, error: charactersError, coverUrl, refresh, removeCharacter,
} = useCharacters()
const { jobs, setPolling } = useTrainingJobs()

const studio = useCharacterStudio()
const {
  ...
  expanding, sheetCostLabel, panelUrl, generateSheet, rerollTile,
  stressBusy, stressTilesFor, stressPassCount, runStressTest,
  markTile, autoReadyIfComplete,
...
function toggleExpanded(c: CharacterRecord) {
  ...
  if (expandedSlug.value) selectedStateId.value[c.slug] = selectedStateId.value[c.slug] || 'default'
}
...
function castInShot(c: CharacterRecord) {
  ...
  const raw = selectedStateId.value[c.slug]
          ...
          <div v-if="expandedSlug === c.slug" class="border-t border-white/[0.06] p-2 space-y-2.5">
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-1.5">
                <button
                  ...
                  :class="(selectedStateId[c.slug] ?? 'default') === v.id
                    ? 'bg-white/20 text-white ring-1 ring-white/30'
                    : 'bg-white/[0.05] text-white/60 hover:bg-white/[0.1]'"
                  :title="draftBadge(v.status) ?? 'Locked — stress-tested'"
                  @click="selectState(c, v.id)"
                >
            ...
            <template v-if="activeState(c)">
              ...
              <div>
                ...
                <input
                  :key="`${activeState(c)!.id}:${activeState(c)!.updatedAt}`"
                  :value="activeState(c)!.descriptor"
                  placeholder="Look descriptor — e.g. soaked navy jacket, wet hair"
                  class="w-full bg-white/[0.04] border border-white/10 rounded px-2 py-1 text-[11px] text-white/85 placeholder-white/30 outline-none focus:border-white/25"
                  @blur="saveDescriptor(c, activeState(c)!, $event)"
              ...
              <div>
                <div class="mb-1 flex items-center justify-between text-[10px] font-medium uppercase tracking-wide text-white/40">
                  ...
                  <span
                    ...
                    :class="statusChipClass(activeState(c)!.status)"
                  >
                    <Check v-if="activeState(c)!.status === 'locked'" class="size-2.5" />
                    {{ statusChipLabel(activeState(c)!.status) }}
                  </span>
                </div>
                <div class="grid grid-cols-5 gap-1.5">
                  ...
                  >
                    <img
                      v-if="panelUrl(activeState(c)!, spec.slot)"
                      :src="panelUrl(activeState(c)!, spec.slot)!"
                    ...
                    <button
                      v-if="panelUrl(activeState(c)!, spec.slot) && !expanding.has(`${c.slug}:${activeState(c)!.id}`)"
                      ...
                      @click="rerollTile(c, activeState(c)!, spec.slot)"
                    >
                      <RefreshCcw class="size-3.5 text-white/85" />
                    </button>
                  </div>
                </div>
                <p v-if="draftBadge(activeState(c)!.status)" class="mt-1 text-[10px] text-white/35">
                  {{ draftBadge(activeState(c)!.status) }}
                </p>
              </div>

              <!-- Stress test: 10-tile identity-fidelity grid that gates Lock -->
              <div v-if="activeState(c)!.sheetImage">
                <div class="mb-1 flex items-center gap-1.5 text-[10px] font-medium uppercase tracking-wide text-white/40">
                  <FlaskConical class="size-3" /> Stress test
                </div>

                <button
                  v-if="!stressTilesFor(c, activeState(c)!)"
                  ...
                  :disabled="stressBusy.has(vkey(c, activeState(c)!))"
                  @click="runStressTest(c, activeState(c)!)"
                >
                  {{ activeState(c)!.status === 'locked' ? 'Re-test' : 'Stress test' }} · ~$0.80
                </button>

                <template v-else>
                  <div class="grid grid-cols-5 gap-1.5">
                    <div
                      v-for="tile in stressTilesFor(c, activeState(c)!)!" :key="tile.idx"
                    ...
                    >
                      ...
                      <div v-if="tile.dataUrl" class="absolute inset-x-0 bottom-0 flex justify-center gap-1 bg-black/55 py-0.5">
                        <button
                          ...
                          title="Recognizable"
                          @click="markTile(c, activeState(c)!, tile.idx, true)"
                        >
                          <Check class="size-3" />
                        </button>
                        <button
                          ...
                          @click="markTile(c, activeState(c)!, tile.idx, false)"
                        >
                          <X class="size-3" />
                        </button>
                      </div>
                  ...
                  <div class="mt-1.5 flex items-center justify-between gap-2">
                    <span class="text-[10px] text-white/45">{{ stressPassCount(c, activeState(c)!) }}/10 recognizable</span>
                    <button
                      ...
                      :disabled="!canLock(stressTilesFor(c, activeState(c)!)!)"
                      @click="autoReadyIfComplete(c, activeState(c)!)"
                    >Lock</button>
                  </div>
                  <p
                    v-if="stressOutcome(stressTilesFor(c, activeState(c)!)!) && !canLock(stressTilesFor(c, activeState(c)!)!)"
              ...
              <div>
                ...
                <div class="grid grid-cols-4 gap-1.5">
                  <div v-for="(f, i) in activeState(c)!.refImages" :key="f" class="group relative aspect-square overflow-hidden rounded">
                    ...
                    <span
                      v-if="i === activeState(c)!.coverIndex"
                    ...
                    <button
                      ...
                      @click="removeRef(c, activeState(c)!, i)"
                    >×</button>
                    <button
                      v-if="i !== activeState(c)!.coverIndex"
                      ...
                      @click="setCover(c, activeState(c)!, i)"
                    >set cover</button>
                  </div>
                  <label class="flex aspect-square cursor-pointer items-center justify-center rounded border border-dashed border-white/15 text-[16px] text-white/40 hover:border-white/30">
                    +<input type="file" accept="image/*" multiple class="hidden" @change="addRefFiles(c, activeState(c)!, $event)">
                  </label>
                </div>
              ...
              <div class="flex items-center gap-1.5">
                <button
                  ...
                  :disabled="expanding.has(`${c.slug}:${activeState(c)!.id}`)"
                  @click="generateSheet(c, activeState(c)!)"
                >
                  <Loader2 v-if="expanding.has(`${c.slug}:${activeState(c)!.id}`)" class="size-3 animate-spin" />
                  <Images v-else class="size-3" />
                  <span>{{ activeState(c)!.panels.length ? 'Regenerate sheet' : 'Generate sheet' }} · {{ sheetCostLabel(c) }}</span>
                </button>
                <!-- Dress: only for non-default looks (the Default is the identity base) -->
                <button
                  v-if="activeState(c)!.id !== 'default'"
                  ...
                  @click="toggleDress(c, activeState(c)!)"
                >
                ...
                <button
                  ...
                  @click="deleteState(c, activeState(c)!)"
                >Delete look</button>
              </div>
              ...
              <div v-if="dressOpen.has(akey(c))" class="rounded border border-white/10 bg-black/20 p-2 space-y-2">
                ...
                <template v-if="dressResult[akey(c)]">
                  ...
                  <div class="flex gap-1.5">
                    <button class="flex-1 rounded bg-white/15 px-2 py-1.5 text-[11px] text-white/90 hover:bg-white/25 cursor-pointer" @click="keepDress(c, activeState(c)!)">Keep as cover</button>
                    <button class="rounded border border-white/15 px-2.5 py-1.5 text-[11px] text-white/60 hover:bg-white/10 cursor-pointer" @click="runDress(c, activeState(c)!)">Retry</button>
                  </div>
                </template>
                <button
                  v-else
                  ...
                  @click="runDress(c, activeState(c)!)"
                >
