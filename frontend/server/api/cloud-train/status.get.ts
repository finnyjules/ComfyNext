 ...
 * Best-effort — a failure here must not fail the poll response, since the
 * weights are already safely on disk by the time this runs.
 */
async function linkTrainedCharacter(displayName: string, weightsFilename: string, trigger: string | null): Promise<void> {
...
export default defineEventHandler(async (event) => {
  ...
  // Human-facing name for the registry record — falls back to outputName
  ...
  if (pred.status === 'succeeded' && outputUrl) {
    ...
    if (await fileExists(localPath)) {
      ...
      if (dl.ok) {
        ...
        if (kind === 'character') {
          await linkTrainedCharacter(displayName, filename, triggerWord || null).catch((err) => {
